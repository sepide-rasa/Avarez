        ...
        public ActionResult Read(StoreRequestParameters parameters)
        {
            ...
            public ActionResult Savee(long HeaderId, prs_SelectHeaderSooratHesab Header,List<prs_SelectDetailSooratHesab> Grid_DetailsArray,  int fldForushandeId, int fldKharidarId,long serial)
            {
                ...
                if (base.Session["TaxUserId"] != null)
                {
                    ...
                    if (HeaderId == 0)
                    {
                        long HeaderIdd = entities.prs_tblSooratHesab_HeaderInsert(new ObjectParameter("fldid", typeof(long)), Header.fldTaxId, new DateTime?(Indatim), Indati2m, new byte?(Header.fldInty), Header.fldinp, Header.fldInno, Header.fldIrtaxId, new long?((long)fldKharidarId), new long?((long)fldForushandeId),
                            Header.fldFunctionName, Header.fldbpn, Header.fldft, Header.fldscln, Header.fldscc, Header.fldcrn, dt, new long?((long)Convert.ToInt32(base.Session["TaxUserId"])));
                    ...
                    {
                        entities.prs_tblSooratHesab_HeaderUpdate(HeaderId, Header.fldTaxId, new DateTime?(Indatim), Indati2m, new byte?(Header.fldInty), Header.fldinp, Header.fldInno, Header.fldIrtaxId, new long?((long)fldKharidarId), new long?((long)fldForushandeId),
                             Header.fldFunctionName, Header.fldbpn, Header.fldft, Header.fldscln, Header.fldscc, Header.fldcrn, dt, new long?((long)Convert.ToInt32(base.Session["TaxUserId"])));
      ...
      public void saveDetail(long HeaderId,List<prs_SelectDetailSooratHesab> Grid_DetailsArray,int state)
        {
            ...
            foreach (prs_SelectDetailSooratHesab hesab2 in Grid_DetailsArray)
            {
                ...
                if (state==1)
                entities.prs_tblSooratHesab_DetailInsert(HeaderId, dt, new long?((long)Convert.ToInt32(base.Session["TaxUserId"])));
                else
                    entities.prs_tblSooratHesab_DetailUpdate(HeaderId, dt, new long?((long)Convert.ToInt32(base.Session["TaxUserId"])));
            }
        }
    }
}
